    ...
    private static List<String> compareWithFixedUnitUpper(Carrier c1, Carrier c2, BigDecimal amount) {
        ...
        if (cat1.equalsIgnoreCase(unit_capacity)) {
            if (cat2.equalsIgnoreCase(unit_capacity)) {
                ...
                String[] upperResult = Util.findBestTimeUnitFormatted(mContext, amount);
                String[] lowerResult = Util.findBestTimeUnitFormatted(mContext, time);
    ...
    private static List<String> compareWithFixedUnitLower(Carrier c1, Carrier c2, BigDecimal amount) {
        ...
        if (cat1.equalsIgnoreCase(unit_capacity)) {
            if (cat2.equalsIgnoreCase(unit_capacity)) {
                ...
                //Upper is electric producer, lower is electric consumer. Calculate how long the producer needs to run with a given amount of time of consuming
                //Amount = Time of consumer => Time of producer = Joule of consumer (watt * time (amount)) / Wattage of producer
                BigDecimal consumer_joule = e2.multiply(amount);
                BigDecimal time = consumer_joule.divide(e1, 2, BigDecimal.ROUND_HALF_UP);
                String[] upperResult = Util.findBestTimeUnitFormatted(mContext, time);
                String[] lowerResult = Util.findBestTimeUnitFormatted(mContext, amount);
            ...
            if (cat2.equalsIgnoreCase(unit_capacity)) {
                //Upper is electric consumer, lower is electric producer. Calculate how long the consumer can run with some time of producing
                //Amount = Time of producer => Time of consumer = Joule of producer (watt * time (amount)) / Wattage of consumer
                BigDecimal producer_joule = e2.multiply(amount);
                BigDecimal time = producer_joule.divide(e1, 2, BigDecimal.ROUND_HALF_UP);
                String[] upperResult = Util.findBestTimeUnitFormatted(mContext, time);
                String[] lowerResult = Util.findBestTimeUnitFormatted(mContext, amount);
                ...
                //Upper is electric consumer, lower is consumer by distance. Calculate how long the consumer needs to consume for the distance of the consumer by distance
                //Amount = Distance in km => Time of consumer = Consumption of consumer on distance (consumption in joule / 100 * amount (distance in km)) / wattage of electric consumer
                BigDecimal volume_consumer_joule = e2.divide(new BigDecimal(100), 10, BigDecimal.ROUND_HALF_UP).multiply(amount);
                BigDecimal time = volume_consumer_joule.divide(e1, 10, BigDecimal.ROUND_HALF_UP);
                String[] upperResult = Util.findBestTimeUnitFormatted(mContext, time);
                ...
                //Upper is electric consumer, lower is energy content by mass. Calculate how much time the electric consumer needs to consume the energy of the lower item with the given amount in kg
                //Amount = Mass of item in kg => Time of consumption = joule of upper item (energy content per kg * amount (kg)) / wattage of electric consumer
                BigDecimal mass_content_joule = e2.multiply(amount);
                BigDecimal time = mass_content_joule.divide(e1, 10, BigDecimal.ROUND_HALF_UP);
                String[] upperResult = Util.findBestTimeUnitFormatted(mContext, time);
                ...
                //Upper is electric consumer, lower is energy content by volume. Calculate how long the consumer needs to run to have the equal energy as the amount of the lower item in litre
                //Amount = volume in litre => Time of consumer = joules of volume (joule per litre * amount (litre)) / wattage of electric consumer
                BigDecimal volume_joule = e2.multiply(amount);
                BigDecimal time = volume_joule.divide(e1, 10, BigDecimal.ROUND_HALF_UP);
                String[] upperResult = Util.findBestTimeUnitFormatted(mContext, time);
            ...
            if (cat2.equalsIgnoreCase(unit_capacity)) {
                //Upper is energy content by mass, lower is electric producer. Calculate how much time of producing is worth how much weight of the upper item
                //Amount = Time of producer => Weight of mass content = Joule of producer (watt * time (amount)) / Mass energy content per kg in Joule
                BigDecimal producer_joule = e2.multiply(amount);
                BigDecimal weight = producer_joule.divide(e1, 10, BigDecimal.ROUND_HALF_UP);
                String[] lowerResult = Util.findBestTimeUnitFormatted(mContext, amount);

                saveAsUpperCompareResult(weight);
                saveAsLowerCompareResult(amount);

                result.add(0, df.format(weight));
                result.add(1, lowerResult[0]);
                result.add(2, com_kg);
                result.add(3, lowerResult[1]);
                return result;
                ...
                //Upper is energy content by mass, lower is electric consumer. Calculate how much time of consuming is worth how much weight of the second item
                //Amount = Time of consumer => Weight of mass content = Joule of consumer (watt * time (amount)) / Mass energy content per kg in Joule
                BigDecimal consumer_joule = e2.multiply(amount);
                BigDecimal weight = consumer_joule.divide(e1, 10, BigDecimal.ROUND_HALF_UP);
                String[] lowerResult = Util.findBestTimeUnitFormatted(mContext, amount);
                ...
                //Upper is energy content by mass, lower is consumer by distance. Calculate the amount of mass of the lower item which is needed to travel the amount of distance with the upper consumer by distance
                //Amount = Distance in km => Mass of lower item to equal distance consumption = Consumption of consumer on distance (consumption in joule / 100 * amount (distance in km)) / joule of mass content per kg
                BigDecimal volume_consumer_joule = e2.divide(new BigDecimal(100), 10, BigDecimal.ROUND_HALF_UP).multiply(amount);
                BigDecimal mass = volume_consumer_joule.divide(e1, 10, BigDecimal.ROUND_HALF_UP);

                saveAsUpperCompareResult(mass);
                saveAsLowerCompareResult(amount);

                result.add(0, df.format(mass));
                result.add(1, df.format(amount));
                result.add(2, com_kg);
                result.add(3, com_km);
                return result;
                ...
                //Upper is energy content by mass, lower is energy content by volume. Calculate how much mass of the upper item is needed to have the same energy as the amount of the lower item in litre
                //Amount = Volume of item in litre => Mass of lower item in kg = joule for volume (volume in litre * amount (litre)) / joule for mass per kg
                BigDecimal volume_content_joule = e2.multiply(amount);
                BigDecimal mass = volume_content_joule.divide(e1, 10, BigDecimal.ROUND_HALF_UP);
