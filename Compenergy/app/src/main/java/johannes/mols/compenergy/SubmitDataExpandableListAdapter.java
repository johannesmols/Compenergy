...
class SubmitDataExpandableListAdapter extends BaseExpandableListAdapter {
    ...
    SubmitDataExpandableListAdapter(Context context, List<String> categories, HashMap<String, List<Carrier>> carriers) {
    ...
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                LayoutInflater inflater = (LayoutInflater) this.mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                convertView = inflater.inflate(R.layout.listview_item_submit_data_layout, null);
            }

            TextView txtListChild;
            txtListChild = (TextView) convertView.findViewById(R.id.fragment_submit_data_list_view_carrier_name);

            /*final CheckBox checkBox = (CheckBox) convertView.findViewById(R.id.fragment_submit_data_toggle);
    ...
    void filterData(String query) {
        ...
        if(query.trim().isEmpty()) {
            ...
            all_categories_list = dbHelper.getCategoryListThatContainsCustoms();
            for(int i = 0; i < all_categories_list.size(); i++) {
                List<Carrier> carriersWithCategoryList;
                carriersWithCategoryList = dbHelper.getCustomCarriersWithCategory(all_categories_list.get(i));
